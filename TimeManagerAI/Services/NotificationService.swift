...
class NotificationService: ObservableObject {
    ...
    func scheduleTaskReminder(for task: TaskEntity) {
        ...
        if !priority.isEmpty {
            ...
            case "red":
                ...
                content.body = "\(task.title ?? "Task" ?? "Task") starts in 15 minutes"
            case "orange":
                content.title = "🟠 MEDIUM PRIORITY"
                content.body = "\(task.title ?? "Task" ?? "Task") starts in 15 minutes"
            case "yellow":
                content.title = "📅 Event Starting Soon"
                content.body = "\((task.title ?? "").isEmpty ? "Event" : task.title ?? "Task") starts in 15 minutes"
            default:
                break
            }
        }
        ...
        content.userInfo = [
            "taskId": task.id?.uuidString ?? UUID().uuidString,
        ...
        center.add(request) { error in
            if let error = error {
                ...
                print("✅ Scheduled reminder for task: \((task.title ?? "").isEmpty ? "Untitled" : task.title ?? "Task")")
            }
        }
    }
    ...
    func scheduleDeadlineWarning(for task: TaskEntity, daysBeforeDeadline: Int) {
        ...
        content.body = "\((task.title ?? "").isEmpty ? "Task" : task.title ?? "Task") is due \(daysText)"
        content.sound = .default

        if task.priority == "red" {
            content.title = "🚨 URGENT DEADLINE"
            content.body = "HIGH PRIORITY: \((task.title ?? "").isEmpty ? "Task" : task.title ?? "Task") is due \(daysText)"
        }
        ...
        center.add(request) { error in
            if let error = error {
                ...
                print("✅ Scheduled deadline warning for task: \((task.title ?? "").isEmpty ? "Untitled" : task.title ?? "Task")")
            }
        }
    }
    ...
    func scheduleIncompleteTaskAlert(for task: TaskEntity) {
        ...
        let timeOverText = "You've been on \((task.title ?? "").isEmpty ? "this task" : task.title ?? "Task") for 30 minutes longer than planned. Still on track?"
        ...
        center.add(request) { error in
            if let error = error {
                ...
                print("✅ Scheduled incomplete task alert for: \((task.title ?? "").isEmpty ? "Untitled" : task.title ?? "Task")")
            }
        }
    }
    ...
    func cancelNotifications(for task: TaskEntity) {
        ...
        print("✅ Cancelled notifications for task: \((task.title ?? "").isEmpty ? "Untitled" : task.title ?? "Task")")
    }
